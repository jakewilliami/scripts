...
df <- tibble::as_tibble(read.csv("~/projects/scripts/julia/Statistics/data-task-zfsd.csv", header = TRUE))

df_pivot <- df %>%
    filter(Correct == 1) %>%
    select(TrialCondition, ResponseTime) %>%
    group_by(TrialCondition) %>%
    dplyr::summarise(average_rt = mean(ResponseTime))

myTheme <- theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (15)),
                 axis.title = element_text(family = "Helvetica", size = (10), colour = "steelblue4"),
                 axis.text = element_text(family = "Courier", colour = "cornflowerblue", size = (10)))

line <- lm(average_rt ~ TrialCondition, data = df_pivot)
line_coeffs = coefficients(line)
equation = paste("italic(y) == ", round(line_coeffs[2],1), "*italic(x) + ", round(line_coeffs[1],1))

ggplot(df_pivot, aes(TrialCondition, average_rt)) +
    geom_point(colour = "orange") +
    scale_x_continuous(limits = c(0, max(df_pivot$TrialCondition))) +
