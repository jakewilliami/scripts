...
# df = DataFrame(CSV.File("data-task-iusd.csv", header = true)) # JC
# df = DataFrame(CSV.File("$(homedir())/Downloads/data-1.csv", header = true))
...
function make_accuracy_pivot(df::DataFrame)
	df_pivot = @from i in df begin
		@select {i.TargetCondition, i.Correct}
		@groupby :TargetCondition
		@collect DataFrame
	end
	df_pivot = combine(groupby(DataFrame(df_pivot), :TargetCondition), :Correct => sum)
