...
function _get_latest_version(::Office2013Singleton, ::WindowsOperatingSystem)
    ...
    # elem = _findfirst_html_tag(doc.root, "id" => "center-doc-outline", tag = :nav)  # superfluous
    # next_elem = _nextsibling(elem, 3)  # superfluous
    note_elem = _findfirst_html_text(doc.root, :p, "The most current version of Office 2013 is", exact = false)
