...
ispower(m::Integer) = ispower_bernstein(Nemo.fmpz(m))

export ispower_bernstein, root_exact

end

module PerfectPowersBruteForce

function ispower(m::Integer)
    for a in 1:m
        for n in 1:m
            a^n == m && return true
        end
...
# module PerfectPowersSlightlyClever
#
# https://math.stackexchange.com/questions/470424/
# https://github.com/trizen/sidef-scripts/blob/master/Math/is_perfect_power.sf
# https://github.com/trizen/perl-scripts/blob/master/Math/is_perfect_power.pl
#
# function ispower(m::Integer)
#     for a in floor(Int, log2(6)):-1:2
#         # if isinteger(m^(1/a))
#         # if powint(rootint($n, $k), $k) == $n
#         if floor(Int, floor(Int, m^(1/a))^a) == m
#             return true
#         end
#     end
#     return false
...
module PerfectPowersHecke

using Hecke

# ispower(m::Integer) = Hecke.ispower(m)

function ispower(n::Integer)
    (n == 0) && return false
...
module PerfectPowersIntegerSequences
...
ispower(n) = isperfectpower(n)

export ispower

end

module PerfectPowersAlex

using Primes

function ispower(n)
    p = (v for (_, v) in factor(n).pe)
    for i in p
        if !(i > 1)
            return false
        end
...
# for P in (:PerfectPowersBruteForce, :PerfectPowersOscar, :PerfectPowersHecke, :PerfectPowersIntegerSequences, :PerfectPowersAlex)
for P in (:PerfectPowersBruteForce, :PerfectPowersHecke, :PerfectPowersIntegerSequences, :PerfectPowersAlex)
    println("Using $P")
    @eval println($P.ispower(m))
end
