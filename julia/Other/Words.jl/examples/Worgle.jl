...
export CharFrequency, SixLetterWord, SixLetterCharFrequencies
...
const SIX_LETTER_WORDS = String[lowercase(word) for word in filter(w -> length(w) == 6 && !any(ispunct(c) for c in w), WORDLIST_SCRABBLE)]
const WORDLIST_TREE = WORDLIST_AS_TREE_SCRABBLE
const TOP_N_WORDS = 5


"Simply an alias for a tuple containing a frequency map for characters in each position of a six-letter word."
const SixLetterCharFrequencies = NTuple{6, OrderedDict{Char, Int}}
...
struct SixLetterWord
    word::String
    letters::NTuple{6, CharFrequency}
    SixLetterWord(word::String, letters::NTuple{6, CharFrequency}) = 
        new(all(islowercase(c) for c in word) ? word : lowercase(word), letters)
end


Base.show(io::IO, flw::SixLetterWord) = print(io, "flw", "\"", flw.word, "\"")


"""
```julia
get_word_score(flw::SixLetterWord, Q::SixLetterCharFrequencies) -> Float64
get_word_score(word::String, Q::SixLetterCharFrequencies) -> Float64

get_word_score(flw::SixLetterWord, V::Vector{SixLetterWord}) -> Float64  # Acts the same as above, uses a list of `SixLetterWord`s (also obtained from `Wordle._construct_frequency_map`).  NOTE: This assumes these six letter words are sorted so that the highest scoring words are at the top!
get_word_score(word::String, V::Vector{SixLetterWord}) -> Float64  # WARN: This method is not supported as searching for each character's positional frequency from a list of `SixLetterWord`s is inefficient without a frequency map.  Consider using method `get_word_score(word::String, Q::SixLetterCharFrequencies)`, or defining this method yourself (hint: use something like `V[findfirst(flw -> flw.letters[i].c == c, V)].letters[i].freq`)

get_word_score(flw::SixLetterWord, W::Vector{String}) -> Float  # Will generate the frequency map from a list of words `W`
# get_word_score(word::String, W::Vector{String}) -> Float64  # ibid.

get_word_score(flw::SixLetterWord) -> Float64  # defaults to using the generated frequency map, `COMMON_SIX_LETTER_WORDS_MAP`
...
"""
get_word_score(flw::SixLetterWord, Q::SixLetterCharFrequencies) =
    round(sum(cf.freq / first(Q[i]).second for (i, cf) in enumerate(flw.letters)) / 6, digits = 3)
get_word_score(word::String, Q::SixLetterCharFrequencies) =
    round(sum(Q[i][c] / first(Q[i]).second for (i, c) in enumerate(lowercase(word))) / 6, digits = 3)
    
get_word_score(flw::SixLetterWord, V::Vector{SixLetterWord}) =
    round(sum(cf.freq / first(V).letters[i].freq for (i, cf) in enumerate(flw.letters)) / 6, digits = 3)
    
get_word_score(flw::SixLetterWord, W::Vector{String}) =
...
"Finds the most common characters for each position of a six letter word; returns a `Vector{SixLetterWord}`, as well as a `NTuple{5, OrderedDict{Char, Int}}`"
...
get_word_score(flw::SixLetterWord) = get_word_score(flw, COMMON_SIX_LETTER_WORDS_MAP)
...
function _previous_word_combinations(words::Vector{SixLetterWord}, n::Int)
    if n > length(words)
    ...
    V = Vector{SixLetterWord}(undef, size(M_permutations, 1))
...
"""
```julia
find_most_common_wordle(V::Vector{SixLetterWord}, wordlist::Vector{String}; ignore::Vector{Char} = Char[]) -> SixLetterWord  # if you give it characters in the `ignore` keyword argument, it will look for words not including those characters
find_most_common_wordle(W::Vector{String}; default::Bool = true, ignore::Vector{Char} = Char[]) -> SixLetterWord  # If `default`, will use the pre-generated (default) frequency list; otherwise will generate the list based on your word list
find_most_common_wordle(; ignore::Vector{Char} = Char[]) -> SixLetterWord  # Uses default/pre-generated information to generate the best word
```
Given a word list, finds the most common word that is a word; returns a tuple of the word itself as a `SixLetterWord`
"""
function find_most_common_wordle(V::Vector{SixLetterWord}, wordlist::Vector{String}; ignore::Vector{Char} = Char[])
    chars_to_ignore = !isempty(ignore)
    top_count = 0
    candidates = SixLetterWord[]
...
"""
```julia
find_most_common_wordle_anagram(V::Vector{SixLetterWord}; ignore::Vector{Char} = Char[]) -> Vector{SixLetterWord}  # if you give it characters in the `ignore` keyword argument, it will look for words not including those characters
find_most_common_wordle_anagram(W::Vector{String}; default::Bool = true, ignore::Vector{Char} = Char[]) -> Vector{SixLetterWord}  # If `default`, will use the pre-generated (default) frequency list; otherwise will generate the list based on your word list
find_most_common_wordle_anagram(; ignore::Vector{Char} = Char[]) -> Vector{SixLetterWord}  # Uses default/pre-generated information to generate the best word
```
Similar to `find_most_common_wordle`, but will try to find full-word acronyms as well, returning a list of these words as a `Vector{SixLetterWord}`
"""
function find_most_common_wordle_anagram(V::Vector{SixLetterWord}; ignore::Vector{Char} = Char[])
    chars_to_ignore = !isempty(ignore)
    top_count = 0
    candidates = SixLetterWord[]
    while top_count <= length(V)
...
"""
```julia
find_most_common_wordles(V::Vector{SixLetterWord}, wordlist::Vector{String}, n::Int; ignore::Vector{Char} = Char[]) -> SixLetterWord  # if you give it characters in the `ignore` keyword argument, it will look for words not including those characters
find_most_common_wordles(W::Vector{String}, n::Int; default::Bool = true, ignore::Vector{Char} = Char[]) -> SixLetterWord  # If `default`, will use the pre-generated (default) frequency list; otherwise will generate the list based on your word list
find_most_common_wordles(n::Int; ignore::Vector{Char} = Char[]) -> SixLetterWord  # Uses default/pre-generated information to generate the best word
```
Similar to `find_most_common_wordle`, except it will return a list of `n` best wordles (`Vector{SixLetterWord}`).
"""
function find_most_common_wordles(V::Vector{SixLetterWord}, wordlist::Vector{String}, n::Int; ignore::Vector{Char} = Char[])
    chars_to_ignore = !isempty(ignore)
    top_count = 0
    candidates = SixLetterWord[]
    words = Vector{SixLetterWord}(undef, n)
    i = 1
    while top_count <= length(V)
        # Check if all characters are unique and that the anagrams are a single word
        if isempty(candidates)
